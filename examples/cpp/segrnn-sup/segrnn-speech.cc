...
unsigned LAYERS = 1;
unsigned INPUT_DIM = 120;
unsigned XCRIBE_DIM = 250;
unsigned SEG_DIM = 64;
unsigned H1DIM = 64;
unsigned H2DIM = 64;
unsigned TAG_DIM = 32;
...
struct UniTrans {
  ...
  explicit UniTrans(Model& model, unsigned layers,
                  unsigned input_dim,
                  unsigned hidden_dim) :
...
struct BiTrans {
  ...
  explicit BiTrans(Model& model, unsigned layers, 
		  unsigned input_dim,
		  unsigned hidden_dim) :
      l2rbuilder(layers, input_dim, hidden_dim, &model),
      r2lbuilder(layers, input_dim, hidden_dim, &model) {
      p_f2c = model.add_parameters({hidden_dim, hidden_dim});
      p_r2c = model.add_parameters({hidden_dim, hidden_dim});
      p_cb = model.add_parameters({hidden_dim});
  }
...
struct SegEmbedUni {
  ...
  explicit SegEmbedUni(Model& m) :
      builder(1, XCRIBE_DIM, SEG_DIM, &m) {
...
//  Parameters* p_i2hb;
  explicit SegmentalRNN(Model& model, dynet::Dict& d_, dynet::Dict& td_) :
      bt(model, LAYERS, XCRIBE_DIM, XCRIBE_DIM), 
      bt1(model, LAYERS, INPUT_DIM, XCRIBE_DIM), 
      bt2(model, LAYERS, XCRIBE_DIM, XCRIBE_DIM) { //, seb(model) {
    d = d_;
    td = td_;
    ...
    // input layer
  //  p_i2h = model.add_parameters({XCRIBE_DIM, INPUT_DIM});
  //  p_i2hb = model.add_parameters({XCRIBE_DIM});
...
//    p_rev2h1 = model.add_parameters({H1DIM, SEG_DIM});
  ...
  vector<Expression> ConstructInput(const vector<vector<float>>& in,
                                  ComputationGraph& cg){
  
    vector<Expression> x1;
    vector<Expression> h2(in.size()/2);
    vector<Expression> xins(in.size()/4);

    for(int i = 0; i < in.size(); ++i){
      x1.push_back(input(cg, {in[i].size()}, &(in[i])));
    }

    vector<Expression> hat_x1 = bt1.transcribe(cg, x1);
    for(int i = 0; i<h2.size(); i++) {
     h2[i] = hat_x1[i*2]; //hat_x1[i];
    }
   
    vector<Expression> hat_x2 = bt2.transcribe(cg, h2);
      ...
      //    auto seg_embedding_ij = seb(i, j-1); // pair<expr, expr>
        //  Expression d = de->embed(j - i);
         ...
         // Expression h1 = rectify(affine_transform({h1b, d2h1, d, y2h1, y, fwd2h1, seg_embedding_ij.first, rev2h1, seg_embedding_ij.second, cf2h1, cf_embeding, ce2h1, ce_embeding}));
          //Expression h1 = rectify(affine_transform({h1b, d2h1, d, y2h1, y, cf2h1, cf_embeding, ce2h1, ce_embeding})); //no segmental embedding
	  Expression h1 = rectify(affine_transform({h1b, y2h1, y, cf2h1, cf_embeding, ce2h1, ce_embeding}));  //no duration embedding 
...
pair<vector<vector<float>>,vector<pair<int,int>>> ParseTrainingInstance(const std::string& line, dynet::Dict& d, dynet::Dict& td, bool test_only = false) {
  ...
  while(1) {
    ...
    boost::algorithm::split( points, word, boost::algorithm::is_any_of( "," ) );
    vector<float> tu;
    for(string point : points){
      tu.push_back(std::stof(point));
    }
    x.push_back(tu);
    if (INPUT_DIM != tu.size()){
      cerr << "assigning input dim from " << INPUT_DIM << " to " << tu.size() << endl; 
      INPUT_DIM = tu.size();
    }
  }
  if(!test_only){
    while(1) {
      in >> word;
      if (!in) break;
      int y = td.convert(word);
      int z = 0; // we don't have the z in speech so we basically dont care
...
void test_only(SegmentalRNN<LSTMBuilder>& segrnn,
	  dynet::Dict& td,
          vector<pair<vector<vector<float>>,vector<pair<int,int>>>>& test_set,
          int max_seg_len = 0)
{
...
void read_file(string file_path,
                     dynet::Dict& d,
                     dynet::Dict& td,
                     vector<pair<vector<vector<float>>,vector<pair<int,int>>>>& read_set,
...
double predict_and_evaluate(SegmentalRNN<LSTMBuilder>& segrnn,
                            const vector<pair<vector<vector<float>>,vector<pair<int,int>>>>& input_set,
                            bool partial,
                            int max_seg_len,
                            string set_name = "DEV"
                            ){
  ...
  double f = 0;
  if(partial) {
    f = evaluate_partial(yz_preds, yz_golds, segrnn.d, segrnn.td);
  } else {
    f = evaluate(yz_preds, yz_golds, segrnn.d, segrnn.td);
  }
 // double f1 = evaluate(yz_preds, yz_golds, segrnn.d, segrn.td);
...
int main(int argc, char** argv) {
  ...
  desc.add_options()
      ...
      ("lstm_dim", po::value<int>(), "the dimension of LSTM hidden units")
  ;
  ...
  if(vm["train"].as<bool>()){
    ...
    dynet::Dict d;
    dynet::Dict td;
    vector<pair<vector<vector<float>>,vector<pair<int,int>>>> training, dev, test;
   ...
   // auto sgd = new AdamTrainer(&model, 1e-6, 0.0005, 0.01, 0.9999, 1e-8);
    ...
    if(vm.count("lstm_dim")){
      XCRIBE_DIM = vm["lstm_dim"].as<int>();
    }

    SegmentalRNN<LSTMBuilder> segrnn(model, d, td);

    if(vm["load_original_model"].as<bool>()){
      cerr << "loading model "<<vm["model_file_prefix"].as<string>() << endl;
   ...
   // int completed_epoch = -1;
    ...
    while(sgd->eta > minimum_lr) {
	  ...
	  double f = predict_and_evaluate(segrnn, dev, vm["partial"].as<bool>(), max_seg_len);
          if (f < f_best) {
            cerr << "old best score = " <<f_best << " new best score = " <<f << endl;
            f_best = f;
            save_models(vm["model_file_prefix"].as<string>(), d, td, model);
          } else { decay = true;}

	  if(decay) {
 	    cerr << "decay the learning rate";
 	    sgd->eta *= eta_decay_rate;
	    sgd->eta0 *= eta_decay_rate;
	  }
          cerr << "**SHUFFLE\n";
      ...
      report++;
      if (report % dev_every_i_reports == 0 && vm["evaluate_test"].as<bool>()){
    ...
    SegmentalRNN<LSTMBuilder> segrnn(model, d, td);
    load_models(vm["model_file_prefix"].as<string>(), model);
    vector<pair<vector<vector<float>>,vector<pair<int,int>>>> test;
    read_file(vm["test_file"].as<string>(), d, td, test, true);
    
    test_only(segrnn, td, test);
  }
